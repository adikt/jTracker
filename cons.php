...
<script language="JavaScript">
    function toggle(id) {
		...
		{
			var answer = confirm("Delete this company Con?")
			if (answer){
		...
		function clearForm(fName, inIndex)
		{
			...
			document.forms[fName].elements["act"].value = "add_cons";
...
Company Cons Manager
</h3>

<div id="linkage" style="display:<?php echo $vis2; ?>; width:120px; margin-left:auto; margin-right:auto;" align="center">
   <a href="#" onClick="clearForm('con_data', <?php echo $_GET['ja']; ?>); toggle('contentdv'); toggle('addnewcon'); toggle('linkage');"><div align="center" class="toolButton"><strong>Add New Con</strong></div></a><BR>
</div>

<div id="addnewcon" style="display:<?php echo $vis1; ?>;" align="center">
	<div>
    ...
    <tbody>
        ...
        <td align="center"><input value="Add" type="submit" name="btnAct"> <input value="Close" type="button" name="btnClose" onClick="toggle('contentdv'); toggle('addnewcon'); toggle('linkage');"></td>
        </tr>
    </tbody>
    </table>
    </form>
...
<?php
	$query = "SELECT * FROM Cons WHERE Cons.company='".$_GET['ja']."' AND Cons.user='".$intUserID."';";
	...
	while($row = mysql_fetch_array($result))
	{
		...
		echo "<td align='right'><img src='".$mURL."images/edit1.png' height='20px' width='20px' style='cursor:pointer;' alt='Edit' onClick=\"editForm('con_data', '".$row['index']."', 'edt_cons', '". $row['title'] ."', '". $row['notes'] ."');\">";
		echo "<img src='".$mURL."images/delete.png' height='20px' width='20px' style='cursor:pointer;' alt='Delete' onClick=\"delForm('".$row['index']."', 'con_data', 'cons')\">";
		echo "</td></tr>";
	}
?>
